...
// Ensure compatibility with the EventItem from admin.events.tsx
interface EventData {
  id: string;
  title: string;
...
export function RegistrationManager({ event }: { event: EventData }) {
  ...
  const confirmMut = useMutation({
    mutationFn: async (regId: string) => {
      ...
      const prefix = event.reg_prefix || "REG";
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.setAttribute("download", `registrations_${event.title}.csv`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-foreground">রেজিস্ট্রেশন ম্যানেজমেন্ট</h3>
        <button
      ...
      {selectedRows.length > 0 && (
        <div className="mb-4 flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 p-3 animate-in slide-in-from-top-2">
